...
void LWallpaperLive2DManager::OnUpdate() const
{
    ...
    if (model->GetModel()->GetCanvasWidth() > 1.0f && width < height)
    {
        ...
        projection.Scale(0.3f, static_cast<float>(width) / static_cast<float>(height)*0.3f);
        // 元のコード ->
    ...
    {
        // サイズ小さく　(灯篭
        projection.Scale(static_cast<float>(height) / static_cast<float>(width) * 0.3f, 0.3f);
        // 元のコード ->
...
// 灯篭が追加
void LWallpaperLive2DManager::changeModel(std::string modelDirectoryName) //const
{
    delete _model;
    _viewMatrix = new CubismMatrix44();
    LoadModel("MangaJK02");
