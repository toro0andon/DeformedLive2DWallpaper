...
#include "LWallpaperLive2DManager.hpp" ///モデル切り替えテストに使った　最終的には使わないので消す (灯篭
...
void LWallpaperModel::Update()
{
    ...
    if (canResetParameter)   //タップされていて、かつ、タップ状態から解放されていない(前回もタップされていた)なら　347行目あたりに定義 (メモ 灯篭
    {
        //ドラッグによる変化
        //ドラッグによる顔の向きの調整
    ...
    else
    {
        ...
        //(灯篭が追加
        //現在のX座標に値が代入されていないなら、float0.0とする
        // モデル初期表示位置はどこかで設定しているはずなので、本当はその初期表示位置の値を代入すべき
        if (!_carrentCoordinateX)
        {
            _carrentCoordinateX = 0.0f;
        }
        //目標座標定義
        _targetWalkCoordinateX = vec.X*2;
        //現在の座標が目標座標より大きいなら　現在の座標-1
        if (_carrentCoordinateX > _targetWalkCoordinateX && !motionUpdated)
        {
            _carrentCoordinateX = _carrentCoordinateX - 0.5f*deltaTimeSeconds;
            //歩くモーション再生
            //-----------------------------------------------------------------
            _model->LoadParameters(); // 前回セーブされた状態をロード
            if (_motionManager->IsFinished())
            {
                // モーションの再生がない場合、groupに設定されているモーションを再生する 優先度は1
                //StartRandomMotion("Hiyori_m01", 1);
            }
            _model->SaveParameters(); // 状態を保存
            //-----------------------------------------------------------------
        }
        ...
        _modelMatrix->SetX(_carrentCoordinateX);
    }
    ...
    //    //モデルの位置を設定
    //    // 範囲を広げるように調整 (灯篭
    //    _modelMatrix->SetX(-_gravitationalAccelerationX);
    //    // 元のコード ->
...
CubismMotionQueueEntryHandle LWallpaperModel::StartMotion(const csmChar* group, csmInt32 no, csmInt32 priority, ACubismMotion::FinishedMotionCallback onFinishedMotionHandler)
{
    ...
    if (!motion) //motionは現在モーションを再生しているかどうかのブール値なのかな？　(灯篭
    {
